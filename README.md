...
If your site sets a `Content-Security-Policy`, it needs to allow the bundle and the API — `script-src https://your-quidchat-host` and `connect-src https://your-quidchat-host`. Nothing else: the widget builds its stylesheet through the CSSOM rather than injecting a `<style>` element, so `style-src 'self'` leaves it looking exactly as intended instead of stripping it back to an unstyled button.
...
`pnpm widget:check http://localhost:3210 <tenant-slug>` builds a page containing nothing but the embed snippet, serves it from its own origin, and drives a browser through the customer's journey: open the widget, ask a question, read the answer and its citation. The separate origin is the point — the server once shipped without CORS headers, so the widget worked from curl, from the test suite, and from no browser at all, and a check that never leaves the same origin cannot see that. Add the page's origin (`http://127.0.0.1:4901`) to the tenant's allowed origins first; the script tells you if you have not.
