...
A business can give its assistant a model from the panel — Settings, Models, AI provider — and is then billed on its own account. Credentials are encrypted with `QUIDCHAT_SECRET_KEY` and never sent back to the browser; a tenant that sets none answers on whatever the server was started with. Environment variables still work and are still the right shape for one business running its own instance.
